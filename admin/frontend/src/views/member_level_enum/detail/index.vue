...
<div class="flex-between-start" style="background-color: #5D5FEF; height: 140px;">
    <div class="flex-c-center-start h-full">
      <div class="m-x-20">
          <span style="font-weight: bold; font-size: 20px; color: #FFFFFF;">会员等级</span>
      </div>
      <div class="m-x-20 m-t-4">
        <span style="font-size: 14px; color: #FFFFFF;">主键、等级名称、描述。</span>
      </div>
    </div>
...
>
<user_info_component
 :params="{memberLevelId:routerQuery.memberLevelEnumId}"
 />
</el-tab-pane>
</el-tabs>
</base-wrapper>
  ...
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      ...
      if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
          ...
          let res = await proxy.$api.member_level_enum['update'](form.value);
          proxy.$modal.msgSuccess(res.message);
          if(res.code == 0){
            refresh();
          }
...
async function getDetail(id) {
    if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
        ...
        let res = await proxy.$api.member_level_enum['get'](id);
        if(res.code == 0){
            form.value = Object.assign({}, res.data);
        }
        else{
...
function refresh(){
    if(routerQuery.memberLevelEnumId){
        getDetail(routerQuery.memberLevelEnumId);
    }
    if(detail.value == 'all'){
        if(routerQuery.memberLevelEnumId){
            getDetailAll(routerQuery.memberLevelEnumId);
        }
    }
}
